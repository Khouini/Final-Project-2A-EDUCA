<html lang="en">
    <?php
        ...
        $check = $mysqli->query("SELECT * FROM datap WHERE ((annee='3') AND (matiere='Français'));") or  die($mysqli->error) ;  
        $check2 = $mysqli->query("SELECT * FROM datap WHERE ((annee='3') AND (matiere='Français'));") or  die($mysqli->error) ;  
    ?>
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>3 Fr Cours</title>
...
?>
    ...
    <h1 class="container mt-4 text-danger fs-1 font-monospace text-center">Les cours:</h1>
            <div style="background-color:CornflowerBlue;" class="container border border-dark border-3 rounded mt-4 bg-gradient">
                <table class="table">
                    <thead>
                        <tr>
                            <th scope="col">#</th>
            ...
            <div>
                <?php while ($rowa = $check2->fetch_assoc()): ?>
                    <div class="container py-3 my-3 border border-4 border-dark">
                    <h1 class="container mt-4 text-dark fs-2 text-center">Cours: <?php echo $rowa['itemName']; ?></h1>
